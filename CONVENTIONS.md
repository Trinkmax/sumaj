...
3. El **worker** (`/worker`, proceso aparte con Baileys) mantiene las sesiones, guarda credenciales en `wa_session_state` (RLS sin policies: solo service role) y publica el QR en `wa_channels.qr`. La app le habla por HTTP (`src/lib/wa/worker.ts`), él le avisa de los entrantes en `POST /api/wa/baileys/events` firmado con HMAC. **El contrato de los dos sentidos es `src/lib/wa/worker-contract.ts`** (copia byte a byte en `worker/src/contract.ts`; si se toca uno se toca el otro):
   - Las sucursales **reciben y mandan todo tipo de mensaje**: texto, imagen, video, audio/nota de voz, documento, figurita, ubicación, contacto y reacciones (`BaileysSendContent`, discriminado por `type`). Del lado de entrada, además del mensaje llegan reacciones, borrados (`revoke`), ediciones (`edit`) y recibos (`status`). Lo que no entra en las columnas de `messages` (cita, reenvío, ver una vez, ubicación, editado, borrado) va en `messages.metadata` (`MessageMetadata` de `lib/types.ts`) y la burbuja lo lee con guards. Lo escrito desde el celular físico se reconoce porque entra con `direction = 'out'`, sin `sent_by` y sin `is_automated`; no hay clave en `metadata` para eso. Lo contrario sí tiene marca: el worker pone `metadata.echo = true` cuando lo que le llega es el eco de algo que mandó él mismo (la app lo deduplica por `wa_message_id`), y la burbuja no lo etiqueta como "desde el celular".
   - **Media por sucursal es asíncrona**: el worker acepta el envío con `pending: true` (HTTP 202), el mensaje queda `pendiente`, y el resultado real llega después como evento `send_result` (ok → `enviado`, error → `fallido` con `error_detail`). Texto, reacción, ubicación y contacto se resuelven en la misma request (`BAILEYS_SYNC_TYPES`). Nunca marcar un adjunto como enviado antes del `send_result`.
   - **Los chats pueden llegar por LID** (Linked ID de WhatsApp) sin número: el evento trae `from: null` y `lid`; la app abre la conversación por `lid` y la completa con el teléfono cuando WhatsApp lo comparte. No asumir que toda conversación de sucursal tiene `wa_id` con dígitos de teléfono. Por la sucursal se le contesta igual (el worker manda al JID `<lid>@lid`, `BaileysSendRequest.toLid`); por el número madre no, la Cloud API solo conoce teléfonos. El LID queda en `contacts.wa_lid` (0032) para reconocer a la persona desde cualquier número.
...
- Roles: admin (todo), vendedor (ve todo, la UI defaultea "míos"), freelance (**solo lo suyo, sin pool** — migraciones 0030/0031). `isStaff` = admin | vendedor.
  - **El dueño de un contacto es el vendedor de su lead** (`leads.assigned_to`, resuelto por `app.contact_owner(agency, contact)`); chats, mensajes, contactos y lo que cuelga de ellos lo heredan (`app.my_*_ids()`). Un lead sin vendedor no lo ve ningún freelance: lo reparte un admin.
  - **`conversations.assigned_to` NO se edita a mano**: lo derivan dos triggers. `conversation_inherit_owner` es `BEFORE INSERT OR UPDATE OF assigned_to` en `conversations` (pisa cualquier valor escrito a mano con el dueño del contacto) y `sync_conversation_owner` corre en `leads` al cambiar el vendedor o la etapa del lead (copia el dueño nuevo a todas las conversaciones del contacto). Asignar un chat = reasignar el lead abierto del contacto; solo los hilos sin lead (difusiones) se asignan directo (`assignConversation` decide eso en el servidor, y la UI relee la conversación después porque el trigger puede haber resuelto otro dueño).
  - Dedupe de contactos por teléfono: RPC `find_contact_by_phone` (vía `findContactByPhone` de `actions/leads.ts`) — busca en TODA la agencia sin mostrar la base, así el freelance no duplica lo que no ve. Nunca comparar contra la lista de la pantalla.
  - Auto-asignación: un lead que cae en una sucursal **sin admin ni vendedor** se asigna al freelance de esa sucursal con menos leads abiertos (`pick_branch_operator(agency, branch)`); si la sucursal tiene staff, queda sin asignar y lo reparte un admin. Derivar sigue siendo admin-only.
  - En la UI: `isStaff` decide el Segmented Míos/Todos del CRM, el filtro "Sin asignar" de la bandeja, Difusiones en el menú y el KPI de Inicio. "Míos" es lo asignado a mí, sin el pool.
